...
public class ImageInstance {
	...
	@Test
	public void ImageInstance() throws Exception {
		
		driver = pubMeth.beforeTest(driver);
		
		pubMeth.rwFile("+++++++++++++++++", "+++++++++++++++++", "+++++++++++++++++");

		CreateInstanceForImage();
		...
		if (reValue) {
			...
			// shut
			WebElement shut = driver.findElement(By.xpath("//span[@data-testid = 'shut']"));
			shut.click();
			Thread.sleep(2000);
			
			// 确定
			WebElement createbk = driver.findElement(By.xpath("//span[@data-testid = 'pop-model-confirm']"));
			createbk.click();
			Thread.sleep(10000);
			
			// 选择关机的服务器
			WebElement servershut = driver.findElement(locator);
			servershut.click();
			Thread.sleep(2000);
			...
			// 制作成镜像
			WebElement createimage = driver.findElement(By.xpath("//span[@data-testid = 'moreOpts-createImage']"));
			createimage.click();
			Thread.sleep(5000);

			// 输入服务器名称
			pubMeth.inputName(driver, "autoimage");	
			
			// 取页面显示的价格
			WebElement pricelast = driver.findElement(By.xpath("//span[@class='price']"));
			priceValue = pricelast.getText();
			Thread.sleep(2000);
			System.out.println("第一次页面取得的价格为=" + priceValue);
			pubMeth.rwFile("第一次页面价格=", priceValue, "");
			
			// 提交
			pubMeth.submit(driver);
			
			// 左侧镜像
			WebElement image = driver.findElement(By.xpath("//a[@data-testid = 'sidebarNav-cloud-image']"));
			image.click();
			Thread.sleep(5000);
			System.out.println("左侧镜像");

			// 选择华东二区
			WebElement huadong2 = driver.findElement(By.xpath("//span[@data-testid = 'region-select-ac2']"));
			huadong2.click();
			Thread.sleep(5000);
	 ...
	 * @throws Exception 
	 */
	private void CreateInstanceForImage() throws Exception {
